...
contract AMM is AccessControl{
	...
	*/
	function tradeTokens( address sellToken, uint256 sellAmount ) public {
		...
		// Calculate the swap amount after fee
		// Fee is feebps/10000 (0.03% if feebps = 3)
		...
		if (sellToken == tokenA) {
			...
			// Calculate tokenB amount to send to user: (qtyB * swapAmt) / (qtyA + swapAmt)
			uint256 buyAmount = (qtyB * swapAmt) / (qtyA + swapAmt);
			require(ERC20(tokenB).transfer(msg.sender, buyAmount), 'Transfer failed');
        
			emit Swap(tokenA, tokenB, sellAmount, buyAmount);
		} else {
			...
			// Calculate tokenA amount to send to user: (qtyA * swapAmt) / (qtyB + swapAmt)
			uint256 buyAmount = (qtyA * swapAmt) / (qtyB + swapAmt);
			require(ERC20(tokenA).transfer(msg.sender, buyAmount), 'Transfer failed');
        
			emit Swap(tokenB, tokenA, sellAmount, buyAmount);
		}
	...
	*/
	function provideLiquidity( uint256 amtA, uint256 amtB ) public {
		...
		// Update invariant after liquidity provision
